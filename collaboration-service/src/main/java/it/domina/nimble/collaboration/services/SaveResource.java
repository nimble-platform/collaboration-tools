...
/**
 * Servlet implementation class SaveResource
 */
@WebServlet(name = "saveResource", urlPatterns = { "/saveResource" })
public class SaveResource extends HttpServlet {
	
	private static final long serialVersionUID = 1L;
    private static final Logger logger = Logger.getLogger(SaveResource.class);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
	        ...
	        if (content.equals("application/json")) {
	            ...
	            SaveResourceType params = SaveResourceType.mapJson(responseString);
	    		Session sess = ServiceConfig.getInstance().getAuth().getPermission(params.getToken(), "saveResource");
	    		if (sess != null) {
	    			if (ServiceConfig.getInstance().saveResource(params, sess)) {
			    		logger.log(Level.INFO, "Resource Saved");
