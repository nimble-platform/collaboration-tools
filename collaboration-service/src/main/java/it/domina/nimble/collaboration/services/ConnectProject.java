...
@WebServlet(name = "connectProject", description = "Connect to an existing Project", urlPatterns = { "/connectProject" })
public class ConnectProject extends HttpServlet {
	
	private static final long serialVersionUID = 1L;
    private static final Logger logger = Logger.getLogger(ConnectProject.class);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
	        ...
	        if (content.equals("application/json")) {
	    		...
	    		ConnectType params = ConnectType.mapJson(responseString);
	    		Session sess = ServiceConfig.getInstance().getAuth().getPermission(params.getToken(), "connectProject");
	    		if (sess != null) {
	    			EProject prj = ServiceConfig.getInstance().getConfig().connectProject(sess, params.getInviteID());
		    		logger.log(Level.INFO, "Project " + prj.getName() + " connected.");
		    		response.getWriter().write(prj.getID().toString());
