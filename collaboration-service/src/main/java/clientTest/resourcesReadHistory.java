...
public class resourcesReadHistory {
			...
			*/
			...
			for (ResourceListType res : lstRes.getChildren()) {
				if (res.getName().equals("IMG_ART333")) {
					...
					ResourceListType history = Functions.getResourceHistory(idToken, projectName, res.getName());
					for (ResourceListType ver : history.getChildren()) {
						System.out.println(ver.getProjectName()+"-"+ver.getName()+"-"+ver.getVersion()+"-"+ver.getNotes());
					} 
